...
def plot_interpolating_and_forecasting_for_models():
    pb_list = ['LV', 'Brusselator', 'Selkov']
    problem_name_mapping = {'LV': 'lotka_voterra', 'Brusselator': 'brusselator', 'FitzHug': 'FitzHugh_Nagumo', 'Selkov': 'selkov', 'SIR': 'sir_problem', 'LV3D': 'three_dim_lotka_voterra', 'gp_ode': 'gp_ode_2d'}
    problem_title = {'LV': 'Lotka-Voterra ($2d$)', 'Brusselator': 'Brusselator ($2d$)', 'FitzHug': 'FitzHugh-Nagumo ($2d$)', 'Selkov': 'Selkov ($2d$)', 'SIR': 'SIR  ($3d$)', 'LV3D': 'Lotka-Voterra ($3d$)', 'gp_ode': 'GP-ODE ($2d$)'}
    ylims = [[(-5, 5), (-3, 3), (-4, 4)], [(-2.5, 7.5), (-0.5, 2.0), (0, 3)]]
    model_title = {'np': 'NP', 'sanodep': 'SANODEP'}
    plt.figure()
    _, axs = plt.subplots(nrows=2, ncols=6, figsize=(9, 3))
    import importlib.util
    for idx, model in enumerate(['np', 'sanodep']):
        ...
        for pb_idx, pb in enumerate(pb_list):
            ...
            # workdir_sanodep = f'exps/experiments/{problem_name_mapping[pb]}/{model}_gp_ode_prior/forcast_prob0.5/seed_1' if pb != 'LV' else f'exps/experiments/{problem_name_mapping[pb]}/model_comparison/{model}_gp_ode_prior/forcast_prob0.5/seed_1'
            dir_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
            workdir = os.path.join(dir_path, f'experiments/gp_ode_2d/{model}/forcast_prob0.5/seed_0' ) # if pb != 'LV' else f'exps/experiments/{problem_name_mapping[pb]}/model_comparison/{model}_gp_ode_prior/forcast_prob0.5/seed_1'
            if idx == 0:
                plot_meta_learn_model(config, workdir, axs[:, :3], pb_idx, '-'.join((model_title[model], problem_title[pb])), ylims)   
            else:
                plot_meta_learn_model(config, workdir, axs[:, 3:], pb_idx, '-'.join((model_title[model], problem_title[pb])), ylims) 
...
# 
    ...
    plt.subplots_adjust(left=0.03, right=0.99, bottom=0.15, top=0.92, wspace=0.25, hspace=0.2)
    script_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    plt.savefig(
        os.path.join(script_dir, 'experiments', 'figs', f"cross_domain_generalization.png"),
        dpi=300,  # Increase dpi for higher resolution
    )


def plot_meta_learn_model(config: ConfigDict, workdir: str, axs, pb_idx, title, ylims):
...
def meta_learn_model_forecasting_plot(**kwargs):
    ...
    for system_idx in range(sample_system_number):
        ...
        axs.set_xlim(t0, t1)
        axs.set_ylim(*ylim)
        axs.set_title(kwargs['title'], fontsize=7)
        axs.grid(True)
        # plt.tight_layout()
        # plt.savefig(
