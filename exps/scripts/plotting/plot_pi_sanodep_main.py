...
def plot_parameter_estimation_from_models(seed, tr_fc_pb):
    ...
    problem_name_mapping = {'LV': 'lotka_voterra', 'Brusselator': 'brusselator', 'Selkov': 'selkov', 'SIR': 'sir_problem', 'LV3D': 'three_dim_lotka_voterra'}
    pb = 'LV'
    ...
    workdir = os.path.join(dir_path, f'experiments/{problem_name_mapping[pb]}/sanodep/forcast_prob{tr_fc_pb}/seed_{seed}' if pb != 'LV' else f'experiments/{problem_name_mapping[pb]}/model_comparison/sanodep/forcast_prob0.5/seed_{seed}')
    plot_meta_learn_model(config, workdir, axs[0], r'SANODEP-$\lambda = 0.5$')

    # Plot the physics enhanced SANODEP
    config_file = os.path.join('exps/cfgs/', problem_name_mapping[pb], f'pi_sanodep.py')
    config = load_config_from_py(config_file)
    config.data.foracsting_problem_prob = 0.5 # this does not matter that much, as we will do forecasting and interpolating anyhow
    workdir = os.path.join(dir_path, f'experiments/{problem_name_mapping[pb]}/pi_sanodep/forcast_prob{tr_fc_pb}/seed_{seed}' if pb != 'LV' else f'experiments/{problem_name_mapping[pb]}/model_comparison/pi_sanodep/forcast_prob0.5/seed_{seed}')
    plot_meta_learn_model(config, workdir, axs, r'PI-SANODEP-$\lambda = 0.5$')
    ...
    plt.savefig(
        os.path.join(script_dir, 'experiments', 'figs', 'param_est.png'), 
        dpi=500,  # Increase dpi for higher resolution
    )


def plot_meta_learn_model(config: ConfigDict, workdir: str, axs, title):
    ...
    # enfoce return_param_est to be True
    if 'PI-SANODEP' in title:
...
def meta_learn_model_interploating_plot(**kwargs):
    ...
    alpha, beta, delta, gamma = np.split(data_params, 4, axis=-1)
    rng = kwargs["rng"]
    model = kwargs["model"]
    ...
    for system_idx in range(sample_system_number):
        ...
        if kwargs['config'].model.name != 'SANODEP':
            mu_params, sigma_params = params_est
            mu_alpha, mu_beta, mu_gamma, mu_delta = np.split(mu_params, 4, axis=-1)
            sigma_alpha, sigma_beta, sigma_gamma, sigma_delta = np.split(sigma_params, 4, axis=-1)
        ...
        # for idx in range(len(all_axes)):
        #     all_axes[idx].grid(True)
