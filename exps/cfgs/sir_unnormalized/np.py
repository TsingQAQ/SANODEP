...
from NeuralProcesses.objectives import sir_unnormalized_obj_func, sir_unnormalized_observer
from NeuralProcesses.models.utils.transformation import Transformation


class SIR_transformation(Transformation):
    def forward(self, x):
        # if x is tensor, we need to convert it to numpy array         
        if hasattr(x, 'numpy'):             
            x = x.numpy()         
        x_aug = np.concatenate([x[..., :1], 0.01 * np.ones_like(x)[..., :1], np.zeros_like(x)[..., :1]], axis=-1)         
        return x_aug

    def backward(self, x):
        """
        :param: transform the initial cond and time back to decision variables 
        """
        return x[..., :1]
...
def get_config():
    ...
    config.model.init_args.x_context = np.zeros(shape=(1, 1, 3))
    config.model.init_args.t_target = np.zeros(shape=(1, 1))
    config.model.init_args.x_target = np.zeros(shape=(1, 1, 3))
    ...
    config.data.dataset_name = 'SIR_Unormalized_ODE'
    ...
    config.data.args.t_range = (0, 1)
    # config.data.args.R0_range = (1.0, 3.0)
    config.data.args.beta_range = (0.1, 2.0)
    ...
    # 3D state dim cases
    config.experimental_design.initial_cond_mapper = SIR_transformation # lambda x: np.concatenate([x, 0.01 * np.ones_like(x), np.zeros_like(x)], axis=-1)
    ...
    config.experimental_design.observer = partial(sir_unnormalized_observer, beta_range = config.data.args.beta_range, gamma_range = config.data.args.gamma_range)
    config.experimental_design.obj_func_form = sir_unnormalized_obj_func

    config.experimental_design.acq_mc_size = 32
    # the following aligns with the ode of lotka_voterra_2d_observer
    config.experimental_design.global_maximum_val = 0.10958
    config.experimental_design.ref_points = np.array([0.51151, 0.79646])
