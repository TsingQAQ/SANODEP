...
# from NeuralProcesses.objectives import brusselator_obj_func, brusselator_observer
...
def get_config():
    ...
    config.data.dataset_name = 'gp_ode_prior'
    config.data.shuffle_buffer_size = 1000
    config.data.num_context_range = (1, 15)
    ...
    config.data.known_traj_range = (1, 20)
    ...
    config.data.args.t_range = (0, 10)
    config.data.args.x_0_range =  ((-1.0, -1.0), (1.0, 1.0))# (np.array([-1.0, -1.0]), np.array([1.0, 1.0]))
    config.data.args.lengthscale_range =  ((0.8, 0.8), (0.8, 0.8)) # (np.array([1.0, 1.0]), np.array([1.0, 1.0]))
    config.data.args.signal_std_range = (1.0, 1.0)
    ...
    config.experimental_design.x0_upper_bound = np.asarray([1.5, 1.5])
    ...
    # config.experimental_design.observer = partial(brusselator_observer, 
    #                                               A_range = config.data.args.A_range, 
    #                                               B_range = config.data.args.B_range)
    # config.experimental_design.obj_func_form = brusselator_obj_func

    config.experimental_design.acq_mc_size = 32
    # the following aligns with the ode of lotka_voterra_2d_observer
    config.experimental_design.global_maximum_val = 0.212877
    config.experimental_design.ref_points = np.array([-1.46742849, 0.3876545])
