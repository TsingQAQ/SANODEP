...
from NeuralProcesses.objectives import sird_observer, sird_obj_func
from NeuralProcesses.models.utils.transformation import Transformation

class SIRDTransform(Transformation):
    @staticmethod
    def forward(x):
        # if x is tensor, we need to convert it to numpy array
        if hasattr(x, 'numpy'):
            x = x.numpy()
        x_aug = np.concatenate([x[..., :1], 0.01 * np.ones_like(x)[..., :1], np.zeros_like(x)[..., :1], np.zeros_like(x)[..., :1]], axis=-1)
        return x_aug
...
def get_config():
    ...
    config.model.name = "PI_SANODEP_SIRD"
    ...
    config.model.cfg_args.latent_d_dim = 3
    config.model.cfg_args.latent_l_dim = 4
    ...
    config.data.aux_eval_metric = {
        "MSE": "SANODEPTestMSELossAcceptBatchData",
        "NLL": "SANODEPNLLLossAcceptBatchData",
    }
    config.data.dataset_name = 'SIRD'
    ...
    config.data.foracsting_problem_prob = (
        0.5  # 1.0: always do forcasting problem, 0.0: always do regression problem
    )

    config.data.args = ml_collections.ConfigDict()
    config.data.args.s_range =  (10, 30) 
    config.data.args.i_range = (0.01, 0.01)
    config.data.args.time_scaling_coefficient = 1.0
    ...
    config.data.args.beta_range = (0.5, 2.0) # (1.2, 1.2) # 
    config.data.args.gamma_range = (0.1, 10.0)  # (0.7, 0.7) # 
    config.data.args.mu_range = (0.1, 5.0)  # (3.0, 3.0) # 
    config.data.args.generator = True

    config.data.args.aux = {"MSE": (20, 100), "NLL": (20, 100)}
    config.data.eval_metrics = {
        "MSE": "SANODEPTestMSELossAcceptBatchData",
        "NLL": "SANODEPNLLLossAcceptBatchData",
    }
    # loss
    ...
    config.loss.likelihood_reg_weight = 0.0
    config.loss.d_penality_weight = 0.0
    ...
    config.training.optimizer.args.peak_lr = (
        1e-3  # not exactly sure how to set these hyperparams
    )
    ...
    # config.training.snapshot_eval = False
    ...
    # 4D state dim cases
    # config.experimental_design.initial_cond_mapper = lambda x: np.concatenate([x, np.zeros_like(x)], axis=-1)
    config.experimental_design.initial_cond_mapper = SIRDTransform # lambda x: np.concatenate([x, 0.01 * np.ones_like(x), np.zeros_like(x), np.zeros_like(x)], axis=-1)
    config.experimental_design.states_num = 4
    config.experimental_design.x0_lower_bound = np.asarray([10.0])
    config.experimental_design.x0_upper_bound = np.asarray([30.0])
    ...
    config.experimental_design.initial_obs_time = np.linspace(config.experimental_design.t0, config.experimental_design.t1, config.experimental_design.maximum_obs_per_traj)
    ...
    config.experimental_design.observer = partial(sird_observer, beta_range=config.data.args.beta_range, gamma_range=config.data.args.gamma_range, mu_range=config.data.args.mu_range)
    config.experimental_design.obj_func_form = sird_obj_func

    config.experimental_design.acq_mc_size = 32
    # the following aligns with the ode of lotka_voterra_2d_observer
    config.experimental_design.global_maximum_val = 0.27187# 0.608
    config.experimental_design.ref_points = np.array([0.52198, 1.04]) # np.array([0.0129, 2.6])
    # config.experimental_design.global_maximum_val = 0.2431# 0.608
    # config.experimental_design.ref_points = np.array([0.5175, 0.8247]) # np.array([0.0129, 2.6])
    # config.experimental_design.global_maximum_val = 0.608
    # config.experimental_design.ref_points = np.array([0.0129, 2.6])
