...
def get_config():
    ...
    config.model.name = 'PI_SANODEP_Brusselator'
    ...
    config.model.cfg_args.param_clamping= partial(distance_penalized_term, 
                                                  lower_bound=np.array([-0.1, 0.0]),
                                                    upper_bound=np.array([2.0, 4.0]))
    # config.model.cfg_args.param_clamping = lambda x: np.clip(x, np.array([-0.1, 0.0]), np.array([2.0, 4.0]))
    ...
    config.training.optimizer.args.learning_rate = 1e-3 # we found this will trigger loss explosion 
