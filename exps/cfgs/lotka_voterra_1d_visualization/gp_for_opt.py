...
class bijective_mapper(Transformation):
    def forward(self, x):
        return np.concatenate([np.repeat(x[..., :1], 2, axis=-1), x[..., 1:]], axis=-1)

    def forward_tf(self, x):
        return tf.concat([tf.repeat(x[..., :1], 2, axis=-1), x[..., 1:]], axis=-1)
...
def get_config():
    ...
    # config.experimental_design.state_observer = None
